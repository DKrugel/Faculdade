...
ger <- function(n = 100, inicial.kick = 0.5, t = 0.16){
  N <- numeric(n)
  values <- numeric(2)
  values[1] <- inicial.kick
  rej <- 0
  ...
  opa <- list(num = N, rejection = rej/n); return(opa)
}

aa <- ger(1000, inicial.kick = 0.5, t = 0.10)
...
#  algoritmo EMMC monte carlo markov chain
# 
